...
from module.savev2 import BackUp
...
class Cursor(object):
    ...
    def __init__(self):
        self.position = 0
        self.folder = "tmp"
        self.file_name = "{}/cursor_combinations.txt".format(self.folder)
...
class Supervisor:
    ...
    def prepare_kwargs_list(self):
        ...
        for selected_variables in combinations(np.arange(data_manager.explanans_size), 3):
...
def parameter_test():
    ...
    supervisor = Supervisor(n_workers=6, output_file='combinations_test', back_up_frequency=100)
